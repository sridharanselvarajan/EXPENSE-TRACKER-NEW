...
function Form() {
    ...
    return (
        <FormStyled onSubmit={handleSubmit}>
            ...
            <div className="form-title"><span className="form-icon">📈</span><span>Add Income</span></div>

            <div className="input-control">
                <label>Title</label>
                <input type="text" value={title} name="title" placeholder="e.g. Monthly Salary" onChange={handleInput('title')} />
            </div>
            <div className="input-control">
                <label>Amount ($)</label>
                <input type="text" value={amount} name="amount" placeholder="e.g. 5000" onChange={handleInput('amount')} />
            </div>
            <div className="input-control">
                <label>Date</label>
            ...
            <button type="submit" className="submit-btn">
                <span>{plus}</span><span>Add Income</span>
            </button>
        </FormStyled>
    )
}
...
const FormStyled = styled.form`
    ...
    .submit-btn {
        ...
        transition: all 0.3s ease; box-shadow: 0 6px 20px rgba(16,185,129,0.3); margin-top: 0.2rem;
        &:hover { transform: translateY(-2px); box-shadow: 0 12px 28px rgba(16,185,129,0.4); }
